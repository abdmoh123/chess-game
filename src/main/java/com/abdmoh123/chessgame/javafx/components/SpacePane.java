...
public class SpacePane extends VBox {
    ...
    public void setPieceImage(char piece_symbol) {
        // use full path because getResource doesn't work with relative (../) paths
        String file_path = "/com/abdmoh123/chessgame/images/pieces/";
        if (Character.isUpperCase(piece_symbol))
            file_path += "white/";
        else
            file_path += "black/";
    ...
    public void highlight(String light_colour, String dark_colour) {
        if (IS_LIGHT)
            setBackground(new Background(new BackgroundFill(Paint.valueOf(light_colour), null, null)));
        else
            setBackground(new Background(new BackgroundFill(Paint.valueOf(dark_colour), null, null)));
    }
    ...
    public void resetBackground() {
        if (IS_LIGHT)
            setBackground(new Background(new BackgroundFill(Paint.valueOf("#ebdbb2"), null, null)));
        else
            setBackground(new Background(new BackgroundFill(Paint.valueOf("#504945"), null, null)));
    }
}
